...
@section('content')
    ...
    <main class="-mt-6">
        ...
        <section id="catalogo-destacado" class="mx-auto max-w-7xl px-6 py-10 lg:px-8">
            ...
            <section class="mt-16 border-y border-[#e8c8c3] py-10" aria-labelledby="kits-sets-title">
                ...
                <div id="kits-carrito-acceso" class="mt-6 hidden justify-end"><a href="{{ route('catalogo.index', ['open_cart' => 'true']) }}" class="inline-flex items-center gap-3 rounded-md bg-[#8f241d] px-5 py-3 font-bold text-white shadow-sm transition hover:bg-[#721d18]">Ver carrito <span id="kits-carrito-contador" class="rounded-full bg-white px-2 py-0.5 text-xs text-[#8f241d]">0</span> <span aria-hidden="true">→</span></a></div>
            </section>
        </section>
    </main>

    <div id="carrito-modal" class="fixed inset-0 z-50 hidden bg-black/50 p-4" role="dialog" aria-modal="true" aria-labelledby="carrito-titulo">
        <div class="ml-auto flex h-full w-full max-w-lg flex-col bg-white shadow-2xl">
            <div class="flex items-center justify-between border-b border-[#e8c8c3] p-5">
                <h2 id="carrito-titulo" class="text-2xl font-black text-[#8f241d]">Tu carrito</h2>
                <button type="button" onclick="cerrarCarrito()" class="text-2xl text-[#8f241d]" aria-label="Cerrar carrito">&times;</button>
            </div>
            <div id="carrito-items" class="flex-1 space-y-3 overflow-y-auto p-5"></div>
            <div class="border-t border-[#e8c8c3] p-5">
                <a href="{{ route('catalogo.index') }}" class="inline-block w-full rounded-md bg-[#b52f25] p-3 text-center font-bold text-white hover:bg-[#8f241d]">Ir a catálogo para solicitar</a>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const leerCarrito = () => {
            try {
                return JSON.parse(window.localStorage.getItem(carritoStorageKey) || '{}');
            } catch {
                return {};
            }
        };
        ...
        window.guardarCarrito = () => {
            window.localStorage.setItem(carritoStorageKey, JSON.stringify(seleccion));
        };

        window.agregar = (id, nombre) => {
            seleccion[id] = seleccion[id] || { id_producto: id, cantidad: 0, nombre };
            seleccion[id].cantidad = Math.min(99, seleccion[id].cantidad + 1);
            guardarCarrito();
            renderCarrito();
        };
        ...
        window.cambiarCantidad = (id, cambio) => {
            ...
            seleccion[id].cantidad += cambio;
            if (seleccion[id].cantidad <= 0) delete seleccion[id];
            guardarCarrito();
            renderCarrito();
        };

        window.establecerCantidad = (id, valor) => {
            if (!seleccion[id]) return;
            seleccion[id].cantidad = Math.max(1, Math.min(99, Number.parseInt(valor, 10) || 1));
            guardarCarrito();
            renderCarrito();
        };
        ...
        window.renderCarrito = () => {
            ...
            if (carritoItems) {
                carritoItems.innerHTML = items.length
                    ? items.map(item => `<div class="flex items-center justify-between gap-3 border-b border-[#f0d7d3] pb-3"><span class="font-semibold text-[#241817]">${item.nombre}</span><div class="flex items-center gap-2"><button type="button" onclick="cambiarCantidad(${item.id_producto}, -1)" class="h-8 w-8 rounded-full bg-[#f7e8e6] font-bold text-[#8f241d]">-</button><input type="number" min="1" max="99" value="${item.cantidad}" onchange="establecerCantidad(${item.id_producto}, this.value)" class="h-8 w-12 border border-[#d9aaa3] text-center font-bold text-[#8f241d]"><button type="button" onclick="cambiarCantidad(${item.id_producto}, 1)" class="h-8 w-8 rounded-full bg-[#b52f25] font-bold text-white">+</button></div></div>`).join('')
                    : '<p class="py-8 text-center text-gray-500">Tu carrito está vacío.</p>';
            }

            document.querySelectorAll('[id^="selector-kit-"]').forEach(selector => {
                const id = Number(selector.id.replace('selector-kit-', ''));
                const item = seleccion[id];
                selector.innerHTML = item
                    ? `<div class="flex items-center gap-2 rounded-md border border-[#e8c8c3] p-1"><button type="button" onclick="cambiarCantidad(${id}, -1)" class="h-8 w-8 rounded bg-[#f7e8e6] font-bold text-[#8f241d]">−</button><input type="number" min="1" max="99" value="${item.cantidad}" onchange="establecerCantidad(${id}, this.value)" class="h-8 w-12 border border-[#d9aaa3] text-center font-bold text-[#8f241d]"><button type="button" onclick="cambiarCantidad(${id}, 1)" class="h-8 w-8 rounded bg-[#b52f25] font-bold text-white">+</button></div>`
                    : `<button type="button" data-agregar-kit data-producto-id="${id}" data-producto-nombre="${selector.dataset.nombre || ''}" class="rounded-md bg-[#b52f25] px-3 py-2 text-sm font-bold text-white hover:bg-[#8f241d]">Agregar</button>`;
            });

            const kitsCarritoAcceso = document.getElementById('kits-carrito-acceso');
            const total = items.reduce((sum, item) => sum + item.cantidad, 0);
            if (kitsCarritoAcceso) {
                kitsCarritoAcceso.classList.toggle('hidden', total === 0);
                kitsCarritoAcceso.classList.toggle('flex', total > 0);
            }

            const kitsCarritoContador = document.getElementById('kits-carrito-contador');
            if (kitsCarritoContador) {
                kitsCarritoContador.textContent = total;
            }
        };
        ...
        document.addEventListener('click', event => {
            const button = event.target.closest('[data-agregar-kit]');
            if (button) agregar(Number(button.dataset.productoId), button.dataset.productoNombre);
        });

        renderCarrito();
        document.getElementById('carrito-modal')?.addEventListener('click', event => {
            if (event.target.id === 'carrito-modal') cerrarCarrito();
        });
