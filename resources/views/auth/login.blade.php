...
<body class="flex min-h-screen items-center justify-center bg-[#173d3a] px-6 text-[#17211f]">
    <main class="grid w-full max-w-4xl overflow-hidden bg-white shadow-2xl md:grid-cols-2">
        <div class="hidden min-h-130 bg-[linear-gradient(135deg,rgba(23,61,58,.9),rgba(9,31,30,.75)),url('https://images.unsplash.com/photo-1581092160607-ee22621dd758?auto=format&fit=crop&w=1200&q=80')] bg-cover bg-center p-10 text-white md:flex md:flex-col md:justify-between"><span class="text-xl font-black tracking-[.2em]">RESMAP</span><div><p class="text-sm font-bold uppercase tracking-[.25em] text-[#f6b941]">Centro de operaciones</p><h1 class="mt-3 text-4xl font-black leading-tight">Todo el trabajo, en un solo lugar.</h1></div></div>
        <div class="p-8 sm:p-12"><a href="{{ route('home') }}" class="text-sm font-bold text-[#23635f]">← Volver al sitio</a><h2 class="mt-12 text-3xl font-black">Acceso interno</h2><p class="mt-2 text-gray-500">Ingresa con tu usuario o correo.</p>
            ...
            <form method="POST" action="{{ route('login.store') }}" class="mt-8 space-y-5">@csrf
                ...
                <button class="w-full bg-[#23635f] p-3 font-bold text-white transition hover:bg-[#173d3a]">Ingresar</button>
            </form>
        </div>
    </main>
</body>
